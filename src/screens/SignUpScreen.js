...
import { signUp } from '../lib/screenOppsApi';
import { colors } from '../constants/theme';

const Schema = yup.object({
  user_name: yup.string().min(1).max(15).required('Username is required'),
...
export default function SignUpScreen() {
  ...
  const handleSubmit = async () => {
    setMessage(null);
    try {
      await Schema.validate({ user_name: userName, email, password }, { abortEarly: false });
      setErrors({});
    } catch (validationErr) {
      const fieldErrors = {};
    ...
    setLoading(true);
    try {
      const info = await signUp({ user_name: userName, email, password });
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View className="flex-1 bg-bg" style={{ paddingTop: insets.top }}>
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 24 }} keyboardShouldPersistTaps="handled">
          ...
          <AuthField label="USERNAME" value={userName} onChangeText={setUserName} error={errors.user_name} />
          ...
          {loading ? (
            ...
            >
              <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 14, color: '#fff' }}>SIGN UP</Text>
            </Pressable>
          )}
          ...
          <Pressable onPress={() => navigation.navigate('Login')} style={{ marginTop: 20, alignItems: 'center' }}>
            <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 13, color: colors.inkFaint }}>
              Already have an account? <Text style={{ color: colors.marquee }}>Log in</Text>
            </Text>
          </Pressable>
        </ScrollView>
      </View>
